...
export default function CommunityPage({ data }) {
  ...
  const loadJoinedTrips = (token) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/joined-trips`, {
      ...
      .then((res) => res.json())
      .then((json) => {
        if (json.data) setJoinedTrips(json.data);
      })
  ...
  const loadGallery = () => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/galleries`)
      .then((res) => res.json())
      .then((json) => {
        if (json.data) setGallery(json.data);
      })
              ...
              <>
                  ...
                  <div className="flex gap-3 md:gap-4">
                    ...
                    <div className="grow min-w-0">
                      <div className="w-full bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-500 mb-3 border border-transparent truncate">
                        Halo {user?.name?.split(" ")[0] || "Pendaki"}, ada
                        rencana muncak?
                      </div>
                ...
                <div className="bg-white rounded-xl shadow-sm flex text-sm font-medium overflow-hidden border border-gray-100 mb-6">
                  <button
                    ...
                    className={`flex-1 py-3 text-center transition-colors ${
                      activeTab === "feed"
                        ? "border-b-4 border-primary text-primary font-bold bg-green-50/30"
                        : "text-gray-500 hover:bg-gray-50"
                    }`}
                  >
                    Cari Barengan
                ...
                {activeTab === "feed" ? (
                  ...
                  <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100">
                    <GalleryGrid gallery={gallery} refreshData={loadGallery} />
                  </div>
                )}
              </>
            )}
            ...
            {activePage === "mytrips" && (
              <div className="space-y-4 md:space-y-6 animate-in fade-in">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button
                    onClick={handleCreateNew}
                    className="bg-primary text-white text-xs md:text-sm font-bold px-3 py-2 md:px-4 md:py-2 rounded-lg hover:bg-green-700 shadow-md transition"
                  >
                    + Buat Baru
                  </button>
                </div>
                ...
                {myTrips && myTrips.length > 0 ? (
                  ...
                  <div className="text-center py-16 bg-white rounded-2xl border-2 border-dashed border-gray-200">
                    <p className="text-gray-400 text-sm">
                      Anda belum membuat ajakan pendakian.
                    </p>
                    <button
                      onClick={handleCreateNew}
            ...
            {activePage === "joined" && (
              <div className="space-y-4 md:space-y-6 animate-in fade-in">
                ...
                {joinedTrips && joinedTrips.length > 0 ? (
                  ...
                  <div className="text-center py-16 bg-white rounded-2xl border-2 border-dashed border-gray-200">
                    <p className="text-gray-400 text-sm">
                      Belum bergabung ke trip manapun.
                    </p>
                    <button
                      onClick={() => setActivePage("feed")}
                      className="mt-4 text-primary font-bold text-sm hover:underline"
